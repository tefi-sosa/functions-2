...
// CODE HERE
let multiply = (num1, num2, multiplyFunction) => {
...
let first = (arr, callbackFunction) => {
  callbackFunction(arr[0]);
}
...
let last = (arr, callbackFunction) => {
  callbackFunction(arr[arr.length - 1])
}
...
// CODE HERE 
let contains = (arr, name, searchName) => {
  searchName(arr.includes(name));
}
...
// CODE HERE
// let uniq = (arr, removeDuplas) => {
//   // noDuplasArr = new Set(arr)
//   // console.log(noDuplasArr)
...
let uniq = (arr, removeDuplas) => {
  noDuplasArr = Array.from(new Set(arr))
  removeDuplas(noDuplasArr)
}
...
let getUserById  = (users, id, userCallBack) => {
  let result =  users.filter((idname) => idname.id.includes(id))
  userCallBack(result[0])
}
