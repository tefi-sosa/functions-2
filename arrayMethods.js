...
// CODE HERE
const totalPopulation  = populations.reduce((x,y) => x + y, 0)
...
const afterTaxOrders = orders.map((order) => order.price * (order.tax + 1));
...
let bobsTotal = purchases.filter(person => person.owner === "Bob").map(price => price.price).reduce((x,y) => x + y, 0);
